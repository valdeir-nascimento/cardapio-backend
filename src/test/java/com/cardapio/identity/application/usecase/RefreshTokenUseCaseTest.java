...
class RefreshTokenUseCaseTest {
    ...
    private final RefreshTokenUseCase useCase = new RefreshTokenUseCase(repo, issuer, tokenHasher, clock);

    @Test
    void rotatesValidToken() {
        UUID subject = UUID.randomUUID();
        RefreshToken stored = RefreshToken.issue(subject, Audience.CUSTOMER,
            tokenHasher.sha256Hex("raw-refresh"), now.minus(Duration.ofMinutes(5)), Duration.ofDays(30));
        when(repo.findByHashedToken(tokenHasher.sha256Hex("raw-refresh"))).thenReturn(Optional.of(stored));
    ...
    void rejectsExpiredToken() {
        ...
        RefreshToken expired = RefreshToken.issue(subject, Audience.CUSTOMER,
            tokenHasher.sha256Hex("raw"), now.minus(Duration.ofDays(40)), Duration.ofDays(30));
